 ...
 * @author suzhida
 */
public class MongoUtil1 implements MongoUtilI {

	// 以下为MongoDB连接配置,根据需要修改
	private String HOST = GetCfg.getValue("MongoDB_Host");
	private int PORT = Integer.parseInt(GetCfg.getValue("MongoDB_Port"));
	private String DATABASENAME = GetCfg.getValue("MongoDB_DBName");
	private String USERNAME = GetCfg.getValue("MongoDB_UserName");
	private char[] PASSWORD = GetCfg.getValue("MongoDB_PassWord").toCharArray();
	
	public static MongoUtil1 mongoUtil = null;
	...
	public static MongoUtil1 getInstance() {
		if (mongoUtil == null) {
			mongoUtil = new MongoUtil1();
		}
		return mongoUtil;
	}
	...
	public void insert(String collectionName, Document document) {
		if (collectionName != null && !"".equals(collectionName) && document != null) {
			MongoCollection<Document> connection = MongoUtil1.getCollection(collectionName);
